      ...
      <div className="container mx-auto px-4 space-y-12">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
            ...
            {/* Animation widget — spans 2 cols */}
            <motion.div
              initial={{ opacity: 0, y: 24 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.7, delay: 0.1, ease: [0.25, 0.4, 0.25, 1] }}
              className="lg:col-span-2 rounded-2xl border border-border overflow-hidden shadow-lg"
              style={{ height: 380 }}
            >
              <AiAnimation />
            </motion.div>

            {/* Content card — spans 3 cols */}
            <motion.div
              initial={{ opacity: 0, y: 24 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ duration: 0.7, delay: 0.15, ease: [0.25, 0.4, 0.25, 1] }}
              className="lg:col-span-3 rounded-2xl border border-border bg-background/80 backdrop-blur-sm p-8 flex flex-col justify-center"
            >
