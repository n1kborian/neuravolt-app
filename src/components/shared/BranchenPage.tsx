...
import { CheckCircle2, ArrowRight, Zap, Shield, FileText, Clock, Building2, UtensilsCrossed, Stethoscope, Hotel, Wrench, ShoppingBag } from "lucide-react";
...
const quickFacts = [
  { icon: Zap,       label: "Ab 4,90 €",       sub: "pro Gerät" },
  { icon: Clock,     label: "24h",              sub: "Antwortzeit" },
  { icon: Shield,    label: "Rechtssicher",     sub: "DGUV V3" },
  { icon: FileText,  label: "Digital",          sub: "Prüfprotokoll" },
];
        ...
        <section ref={heroRef} className="w-full py-16 md:py-24">
          <div className="container mx-auto px-4">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
              ...
              >
                {quickFacts.map(({ icon: FIcon, label, sub }, i) => (
                  <motion.div
                    key={label}
                    initial={{ opacity: 0, scale: 0.94 }}
                    animate={heroIn ? { opacity: 1, scale: 1 } : {}}
                    transition={{ duration: 0.4, delay: 0.2 + i * 0.07 }}
                    className="rounded-2xl border border-border bg-background/80 backdrop-blur-sm p-5 shadow-lg flex flex-col gap-2"
                  >
                    <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-border bg-background shadow-sm">
                      <FIcon className="h-4 w-4 text-foreground" />
                    </div>
                    <p className="text-xl font-bold text-foreground leading-none">{label}</p>
                ...
                <motion.div
                  ...
                  className="col-span-2 rounded-2xl border border-border bg-background/80 backdrop-blur-sm p-5 shadow-lg flex items-center gap-4"
                >
                  ...
                  <div>
                    <p className="text-sm font-semibold text-foreground">Branchenspezifische Prüfung</p>
                    <p className="text-xs text-muted-foreground mt-0.5">Alle Geräteklassen · kein Aufwand Ihrerseits</p>
                  </div>
                  <div className="ml-auto flex items-center gap-1.5 rounded-full border border-border bg-muted/50 px-2.5 py-1 shrink-0">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {geraete.map((g, i) => (
                <motion.div
                  ...
                  className="group flex items-center gap-3 rounded-xl border border-border bg-background/80 backdrop-blur-sm p-4 shadow-lg transition-all duration-300 hover:border-foreground hover:shadow-xl"
                >
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {besonderheiten.map((b, i) => (
                ...
                >
                  <div className="flex h-9 w-9 items-center justify-center rounded-xl border border-border bg-background shadow-sm mb-4 transition-all duration-300 group-hover:border-brand group-hover:bg-brand group-hover:text-brand-foreground">
