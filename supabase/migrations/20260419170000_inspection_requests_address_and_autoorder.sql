-- Erweitert inspection_requests um Adresse der Durchführung und
-- erzeugt automatisch einen orders-Eintrag, sobald eine Anfrage eintrifft,
-- damit der Auftrag sofort im Partner-Dashboard sichtbar ist.
...
begin
  ...
  -- Mengen­staffel (einfache Festpreis-Logik)
  ...
  -- Kundendaten aus Profil + auth.users
...
drop trigger if exists inspection_requests_autoorder on public.inspection_requests;
create trigger inspection_requests_autoorder
  after insert on public.inspection_requests
  for each row execute function public.create_order_from_inspection_request();
