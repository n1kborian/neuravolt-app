...
const PHASE_MS = [6500, 1400, 2200, 2400, 2000, 3000, 1800, 3800, 3200, 4000, 5000];
const TRANSITION_MS = 2000;

const PHASE_LABELS = [
  "Auftragsübersicht",
  "Analyse gestartet …",
  "Standorte werden markiert …",
  "Sortierung nach Standort …",
  "Zoom: München",
  "Overlap erkannt · KW 17",
  "Kritischen Auftrag auswählen …",
  "KI-Optimierungsvorschlag wird gesendet …",
  "Antwort erhalten ✓",
  "Termin aktualisiert · Cluster komplett ✓",
...
export function AiAnimation() {
  ...
  // Phase timer — shows blur transition scene between phases
  useEffect(() => {
    if (transitioning) {
      const t = setTimeout(() => {
  ...
  return (
    <div className="relative w-full h-full rounded-2xl border border-border bg-background overflow-hidden flex flex-col">
      ...
      {/* ── Transition overlay — full blur between phases ─────────────────── */}
      <AnimatePresence>
        {transitioning && (
          <motion.div
            className="absolute inset-0 z-50 flex flex-col items-center justify-center"
            style={{ backdropFilter: "blur(18px)", background: "rgba(255,255,255,0.30)" }}
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
